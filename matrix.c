...
void matrix_add(matrix_t* RESTRICT const c, const matrix_t* RESTRICT const a, const matrix_t* RESTRICT const b)
{
    uint_fast16_t i, j;
    const uint_fast8_t arows = a->rows;
    const uint_fast8_t acols = a->cols;
    ...
    const matrix_data_t* RESTRICT const adata = a->data;
    const matrix_data_t* RESTRICT const bdata = b->data;
    matrix_data_t* RESTRICT const cdata = c->data;
    ...
    for (i = 0; i < arows; ++i)
    {
        for (j = 0; j < acols; ++j)
        {
            cdata[i * acols + j] = adata[i * acols + j] + bdata[i * acols + j]; 
        }
    }
}
